...
Original file is located at
    https://colab.research.google.com/drive/1qSwumxewxvPMeInqIefD5fFBQ8OpLBhA
"""
...
# From paper the sequence of Architecture A (Cha-Inspired architecture):
#1. 2D conv of size 25x25, filter depth= 3, number= 24, padding = same stride=1
#2. Max Pool of size 5x5, depth= 24, stride= 2
#3. 2D conv of size 15x15, depth 3, num = 48, stride =1
#4. Max Pool  of size 2x2, depth = 48, stride= 2
#5. 2D Conv of size 11x11, depth = 3, num= 96, stride =1
#6. Flatten
#7. Dropout--0.5
#8. ReLU Activation
#9. Dense
...
# Define parameters for Architecture A
model= Sequential()
model.add(Convolution2D(24, 25, strides=(1,1),
                           padding='same',
                           input_shape=input_shape))
model.add(MaxPooling2D(pool_size =(5,5), strides=(2,2)))
model.add(Convolution2D(48, 15, strides=(1,1),padding="same"))
model.add(MaxPooling2D(pool_size =(2,2), strides=(2,2)))
model.add(Convolution2D(3, 11, strides=(1,1),padding="same"))
model.add(Flatten())
...
#model.save(ArchA_model.h5)
model.summary()

model.save_weights("./ArchA_model.h5")

model.save("./archi_A", include_optimizer=True)
...
with open("./ArchA_model.json","w") as json_file:
  json_file.write(model_json)

from keras.models import model_from_json
# opening and storing file in a variable
json_file = open('./ArchA_model.json','r')
...
mymodel.load_weights("./ArchA_model.h5")
